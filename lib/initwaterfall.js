...
exports.import = function importTSV(config, country, previousStatus, callback){
	...
	var parseOptionsMin = {delimiter: "\t"};	
		
	fs.exists(countryDataFilePath, function(exists){
		console.log(exists ? "it's there" : 'not there!');

		try{
			
			var readStream = fs.createReadStream(countryDataFilePath);
			console.log(readStream);
			var parser = parse(parseOptionsMin, function(err, data){
  				console.log(err ? "parsing error" : 'not there!');
				console.log(data);
				callback(data);
			});
...
//	fs.createReadStream(countryDataFilePath).pipe(parser);
