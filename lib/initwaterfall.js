...
exports.testparse = function testparse(config,country, previousStatus){
	...
	var parseOptions = {delimiter: "\t",quote: false,columns : config.datacolumns.countries};

	var myparse = require('csv').parse;
	var mystream = fs.createReadStream(countryDataFilePath);

	mystream.pipe(myparse(parseOptions));

	var had_error = false;
	mystream.on('data', function(err){
		console.log("data ");
	});
	mystream.on('error', function(err){
		console.log("error " + err);
	});
	mystream.on('finish', function(data){
		console.log("finish");
	});
	mystream.on('close', function(data){
		console.log("close");
	});
...
exports.import = function importTSV(config, country, previousStatus, callback){
