...
var configFileCheck = function (configFile, callback) {
	...
	doesFileExist(configFile,function(err, status){
		...
		console.log("configFileCheck::doesFileExist err=" + err);
		console.log("configFileCheck::doesFileExist status=" + status);
		
		if (err || (status==false)){
			callback(new customError.FileNotFound(configFile + " not found"));
		}
	});
