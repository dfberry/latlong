...
function eachCountry(config, country, cb){
	...
	async.waterfall([
		...
		function(downloadStatus, callback){
			...
			if (JSON.parse(downloadStatus).download.status==true){

				unzip(config, country, downloadStatus, function(error, results){
					if (error) {
						callback(error);
					} else {
				...
				var unzipStatus = "{\"status\": \"false\", \"error\": \"no file to unzip\"}";
				countryStatus += "{ \"unzip\" : " + unzipStatus + "}";		
				callback(null, unzipStatus);				
			}
		}/*,
...
function unzip(config, country, previousStatus, callback){
	...
	if (previousStatus==false){
		callback(null, "{\"status\": \"false\", \"file\": \"" + zipfile + "\", \"error\": \"not downloaded\"}");
