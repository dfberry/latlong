...
var unzipFiles = function(config, statusArray, callback){
	...
	var files = Object.keys(statusArray );

	async.each(files, function(countryFile,done) {
		...
		console.log("unzip file:" + config.datadirectory + countryFile);
		console.log(countryFile);
		/*			
		unzip(config.datadirectory + countryFile)
		.then(function(timeToDownload){
