...
var downloadFiles = function(config, statusArray, callback){
	...
	async.each(config.country, 
		...
		download(config.dataurl + country, config.datadirectory + country, function(timeToDownload){
	...
	http.get(url, function (response) {
		...
		if (response.statusCode==200){
			...
			file.on('finish', function () {
				...
				file.close(deferred.resolve(callback(endTime)));  // close() is async, call cb after close completes.
				return deferred.promise;
			});
		} else {
